...
class CategoryManagementView extends StatefulWidget {
  ...
  @override
  State<CategoryManagementView> createState() =>
...
class _CategoryManagementViewState extends State<CategoryManagementView> {
  ...
  Future<void> _handleDelete(CategoryModel cat) async {
    ...
    // Async gap — do NOT use `context` after this line.
    final count = await AppDatabase.countTransactionsByCategory(cat.id);

    // FIX: check mounted after the await before using context again.
    if (!mounted) return;
        ...
        actions: [
          ...
          TextButton(
            onPressed: () {
              ...
              catProvider.remove(cat.id);
            },
            child: Text(loc.str('delete'),
  ...
  void _openForm({CategoryModel? editCat}) {
  ...
  Widget build(BuildContext context) {
    ...
    // FIX: previous filter for 'both' was inverted — a category of type
    // 'both' was included for 'income' and 'outcome' filters but the
    // logic was confusing and had an extra negation. Simplified:
    //   all    → show everything
    //   income → show income-only and both
    //   outcome→ show outcome-only and both
    final shown = cats.where((c) {
      if (_filter == 'all') return true;
      return c.type == _filter || c.type == 'both';
    }).toList();
    ...
    return Scaffold(
      ...
      body: Column(children: [
        ...
        Padding(
          ...
          child: Row(
            children: ['all', 'income', 'outcome'].map((f) => GestureDetector(
              ...
              return Padding(
                ...
                child: Row(children: [
                  ...
                  IconButton(
                    ...
                    onPressed: () => _openForm(editCat: cat),
                  ),
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        child: Column(children: [
          ...
          // Type
          _FormSection(
            label: loc.str('categoryType'),
            child: Container(
              decoration: BoxDecoration(
                  color: AppColors.divider,
                  borderRadius: BorderRadius.circular(AppRadius.chip)),
              child: Row(
                children: ['outcome', 'income', 'both'].map((t) => Expanded(
