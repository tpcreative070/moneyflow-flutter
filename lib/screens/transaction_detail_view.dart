...
class TransactionDetailView extends StatelessWidget {
  ...
  /// Safe date parse — returns a fallback so the UI never crashes.
  ...
  Widget build(BuildContext context) {
    ...
    // FIX: use tryParse on every date field to prevent crashes on
    //      malformed ISO strings stored in older records.
    final txDate     = _parseDate(tx.date);
    final createdAt  = _parseDate(tx.createdAt);
    final updatedAt  = _parseDate(tx.updatedAt);

    // Detail rows — conditionally include sync status and updatedAt.
    final rows = <(IconData, String, String)>[
      (Icons.calendar_today, loc.str('date'),
      DateFormat.yMMMMEEEEd().format(txDate)),
      (Icons.category, loc.str('category'), tx.categoryName),
      if (tx.note.isNotEmpty)
        (Icons.notes, loc.str('note'), tx.note),
      // FIX: hide sync status row for guest users — it's irrelevant.
      if (!isGuest)
      ...
      // FIX: updatedAt was present in Swift but missing here — add it.
      (Icons.update, loc.str('updatedAt'),
                ...
                Padding(
                  ...
                  child: Container(
                    decoration: BoxDecoration(
                      ...
                      borderRadius:
                      BorderRadius.circular(AppRadius.heroCard),
                    ),
